...
import pandaPagination from '../src/pagination.vue';

describe('测试Pager组件', () => {
	// 测试是否能够渲染正确的页码数量
	it('渲染正确的页码数量', async () => {
		const wrapper = mount(pandaPagination, {
			props: {
				currentPage: 1,
				total: 20,
			},
	...
	it('页码边界是否正常', async () => {
		...
		expect(wrapper.find('a.pandaui-icon-arrow-left').classes()).toContain(
			'disabled'
		);
		expect(wrapper.findAll('a')[0].classes()).toContain('disabled');

		expect(wrapper.find('a.pandaui-icon-arrow-right').classes()).not.toContain(
			'disabled'
		);
		...
		expect(wrapper.find('a.pandaui-icon-arrow-right').classes()).toContain(
			'disabled'
		);
		expect(wrapper.find('a.pandaui-icon-arrow-left').classes()).not.toContain(
			'disabled'
		);
	});
});
